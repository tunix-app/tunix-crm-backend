...
export class ClientService {
    ...
    async createClient(createClient: CreateClientDto) {
        ...
        const trainer = await this.knexService.db('users').where({ id: createClient.trainer_id }).first();
        const clientMetadata = await this.knexService.db('users').where({ id: createClient.client_id }).first();


        if (!trainer) {
            throw new BadRequestException(`Trainer with id ${createClient.trainer_id} does not exist`);
        } else if (!clientMetadata) {
            throw new BadRequestException(`Client with id ${createClient.client_id} does not exist`);
        } else {
            const newClient = await this.knexService.db('clients').insert(createClient).returning('*');
            if (!newClient) {
